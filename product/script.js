...
const displayData = async () => {
  try {
    ...
    const price = product.redPrice
      ? product.redPrice.price
    ...
    productDetails.innerHTML = `
                                <div class="product-images">
                                    <img
                                        src="${
                                          product.fabricSwatchThumbnails[0]
                                            .baseUrl
                                        }"
                                        class="product-big-image" alt="${
                                          product.name
                                        }"
                                    />
                                    <ul>
                                ...
                                <div class="product-details">
                                    ...
                                    <div class="price-container">
                                        <div>
                                            <span class="product-price" style="color:${
                                              !product.redPrice ? "#111" : ""
                                            }">${price}$</span>
                                            ${
                                              product.redPrice
                                        ...
                                        ${
                                          product.percentageDiscount
                                            ? `<span class="discount">${product.percentageDiscount}</span>`
                                            : ""
                                        }
                                    </div>
                                    ...
                                    <div class="colors-container">
                                        ${data.product.articlesList
                                          .map((e, index) => {
                                            ...
                                            return `<input onchange="changeColorHandle(${index})" type='radio' name='color' id='color-${index}' value='${
                                              e.color.rgbColor
                                            }' class="colors-radio" ${
                                              index === 0 ? "checked" : ""
                                            }><label style="background-color:${
                                              e.color.rgbColor
                                            }" for="color-${index}"></label>`;
                                          })
                                          .join("")}
                                    </div>
                                    <h5>Size</h5>
                                    <div class="sizes-container">
                                        ${product.variantsList
                                          .map((e, index) => {
                                            return `<input type='radio' name='size' id='${
                                              e.size.name
                                            }' value='${
                                              e.size.name
                                            }' class="sizes-radio" ${
                                              index === 0 ? "checked" : ""
                                            }><label for="${e.size.name}">${
                                              e.size.name
                                            }</label>`;
                                          })
                                          .join("")}
...
const handleAddItem = () => {
  let productIdIndex;
  document
    .querySelectorAll('.colors-container input[type="radio"]')
    .forEach((e, i) => {
      if (e.checked) {
        productIdIndex = i;
      }
  ...
  const cartItem = cartContent.find(
    (item) => item.id === globData.product.articlesList[productIdIndex].code
  );
  if (cartItem) {
    cartItem.quantity++;
  } else {
    const product = {
      ...
      price: globData.product.redPrice
        ? globData.product.redPrice.price
        : globData.product.whitePrice.price,
      image: document.querySelector(".product-big-image").src,
      color: document.querySelector(
        '.colors-container input[type="radio"]:checked'
      ).value,
      size: document.querySelector(
        '.sizes-container input[type="radio"]:checked'
      ).value,
      quantity: 1,
    };
